...
initUndistortRectifyMap(cameraMatrix, 
				distCoeffs, 
				R1*R0,//the rotation matrix p_new=R*p_old
                                getOptimalNewCameraMatrix(	cameraMatrix, 
								distCoeffs, 
								imageSize, 
								1, 
...
initUndistortRectifyMap(cameraMatrix, 
				distCoeffs, 
				R2*R0,//the rotation matrix p_new=R*p_old
                                getOptimalNewCameraMatrix(	cameraMatrix, 
								distCoeffs, 
								imageSize, 
								1, 
...
initUndistortRectifyMap(cameraMatrix, 
				distCoeffs, 
				R3*R0,//the rotation matrix p_new=R*p_old
                                getOptimalNewCameraMatrix(	cameraMatrix, 
								distCoeffs, 
								imageSize, 
								1, 
...
[gx,gy]=grad(ref)
//[dimg/dx*dx/dr<whatever>+dref/dy*dy/dr<whatever>]
//this thing is a Numpixels x 3 matrix
dimg_dr=[gx(:).*dx_dr1(:)+gy(:).*dy_dr1(:)     gx(:).*dx_dr2(:)+gy(:).*dy_dr2(:)     gx(:).*dx_dr3(:)+gy(:).*dy_dr3(:)];


//solve dimg/dr * dr = ref - img
